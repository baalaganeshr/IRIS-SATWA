...
export default function DamageScan({ onNavigate }: DamageScanProps) {
  ...
  return (
    <div className="space-y-6 animate-enter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          {result ? (
            <>
              ...
              <div className="navy-card p-5 animate-fade-in-up">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {/* Confidence score */}
                  <div className="bg-navy-950/60 rounded-xl border border-navy-700/40 p-3.5 text-center">
                    <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mb-1.5">Confidence</p>
                    <p className={`text-2xl font-black font-mono ${confColor(result.crack_confidence)}`}>
                  ...
                  {/* Severity label */}
                  <div className="bg-navy-950/60 rounded-xl border border-navy-700/40 p-3.5 text-center">
                    <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mb-1.5">Severity</p>
                    <p className={`text-base font-extrabold ${confColor(result.crack_confidence)}`}>
                  ...
                  <div className="bg-navy-950/60 rounded-xl border border-navy-700/40 p-3.5 text-center">
                    ...
                    <p className={`text-xs font-semibold ${result.crack_confidence > 0.45 ? "text-red-400" : "text-emerald-400"}`}>{result.crack_confidence > 0.45 ? "Damage Detected" : "No Major Damage"}</p>
                  </div>
                </div>
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-[10px] font-mono font-bold ${confColor(result.crack_confidence)}`}>{(result.crack_confidence * 100).toFixed(1)}%</span>
                  </div>
                ...
                {/* Note */}
                <div className="bg-navy-950/40 rounded-lg p-2.5 border border-navy-700/30">
                ...
                <div className="mt-4 flex gap-3">
                  <button
                    ...
                    disabled={injecting}
                    className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-violet-600/20 to-violet-500/10 hover:from-violet-600/30 hover:to-violet-500/20 disabled:opacity-40 disabled:cursor-not-allowed text-violet-300 font-bold text-xs py-2.5 rounded-xl transition-all border border-violet-500/25 cursor-pointer"
                  >
                    {injecting ? (
            ...
            /* Empty state */
            <div className="navy-card p-8 flex flex-col items-center justify-center min-h-[280px] animate-fade-in-up">
              <div className="w-14 h-14 rounded-2xl bg-navy-800/60 border border-navy-700/40 flex items-center justify-center mb-4">
                <svg className="w-6 h-6 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.04 12.32a1.01 1.01 0 010-.64C3.42 7.51 7.36 4.5 12 4.5c4.64 0 8.57 3.01 9.96 7.18.07.21.07.43 0 .64C20.58 16.49 16.64 19.5 12 19.5c-4.64 0-8.57-3.01-9.96-7.18zM15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </div>
              <p className="text-sm font-bold text-slate-500 mb-1">No Analysis Yet</p>
              <p className="text-xs text-slate-600 text-center max-w-xs">Upload a structural image and click "Analyze" to detect surface damage using YOLOv8 AI.</p>
            </div>
          )}
      ...
      {/* ── Image Comparison (full width, below the grid) ── */}
      {result && (preview || result.overlay_image_base64) && (
        <div className={`grid gap-5 animate-fade-in-up ${preview && result.overlay_image_base64 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 max-w-2xl"}`} style={{ animationDelay: "100ms" }}>
          {preview && (
            <div className="navy-card p-4">
              ...
              <img src={preview} alt="Upload preview" className="w-full rounded-lg border border-navy-700/50 object-contain max-h-[280px] bg-navy-950" />
            </div>
          )}
          {result.overlay_image_base64 && (
            <div className="navy-card p-4">
              <h3 className="section-title mb-3">
                ...
                Damage Overlay — Highlighted Regions
              </h3>
              <img
                src={`data:image/png;base64,${result.overlay_image_base64}`}
                alt="Damage overlay"
                className="w-full rounded-lg border border-red-500/20 object-contain max-h-[280px] bg-navy-950"
              />
              <p className="text-[10px] text-slate-600 mt-2 text-center">Red regions indicate detected anomaly density (potential cracks)</p>
            </div>
          )}
        </div>
      )}

      {/* ── Injection Result — Full Damage Assessment Report (full width) ── */}
      {injectResult && (() => {
            ...
            return (
              <div className={`navy-card overflow-hidden animate-fade-in-up ${sc.border}`} style={{ animationDelay: "200ms" }}>
                ...
                <div className="p-6 space-y-5">
                  ...
                  {/* ── Two-column: Left (Impact + Urgency + Breakdown) | Right (Actions) ── */}
